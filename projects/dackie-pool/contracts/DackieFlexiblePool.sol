...
contract DackieFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 cakeAtLastUserAction; // keeps track of dackie deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Cake token
    IDackiePool public immutable cakePool; //Cake pool
    ...
    //When call dackiepool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositDackie(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    /**
     * @notice Constructor
     * @param _token: DACKIE token contract
     * @param _dackiePool: DackiePool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_dackiePool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Dackie Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in DACKIE)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        emit DepositDackie(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Dackie Flexible Pool
    ...
    /**
     * @notice Withdraws from Dackie Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking dackie");
        staking = false;
        cakePool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Dackie Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in DackiePool
     */
    ...
    /**
     * @notice Deposits tokens into DackiePool to earn staking rewards
     */
