...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _dackieToken: the address of the dackie token
     */
    function initialize(
        address _admin,
        address _farmBooster,
        address _masterchefV2,
        address _dackieToken
    ) external {
        ...
        dackieToken = IERC20(_dackieToken);
    }
    ...
    function harvestCake() internal {
        uint256 dackieBalance = dackieToken.balanceOf(address(this));
        if (dackieBalance > 0) {
            dackieToken.safeTransfer(msg.sender, dackieBalance);
        }
    }
}