...
import "./interfaces/IDackiePadV6.sol";
import "./utils/WhiteList.sol";
import "./IDackie.sol";

/**
 * @title DackiePadInitializableV6
 */
contract DackiePadInitializableV6 is IDackiePadV6, ReentrancyGuard, Whitelist {
    ...
    function initialize(
        ...
        iDackie = IDackie(_iDackieAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        if (!_poolInformation[_pid].isSpecialSale) {
            // getUserCredit from IDackie contract
            uint256 idoCredit = iDackie.getUserCredit(msg.sender);
            require(userCreditUsed[msg.sender].add(_amount) <= idoCredit, "Not enough IDO credit left");
