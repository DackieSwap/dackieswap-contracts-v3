...
import "./interfaces/IDackiePad.sol";
import "./utils/WhiteList.sol";
import "./interfaces/IDackieTier.sol";

/**
 * @title DackiePadByTier
 */
contract DackiePadByTier is IDackiePad, ReentrancyGuard, Whitelist {
    ...
    address public immutable DACKIE_PAD_FACTORY;
    ...
    // IDackie contract
    IDackieTier public iDackieTier;
    ...
    uint256 public tier5;

    uint256 public tier0Amount;
    ...
    uint256 public tier4Amount;

    uint256 public tier5Amount;
     ...
     * @param _iDackieAddress: the address of the IDackie
    ...
    function initialize(
        ...
        address _iDackieAddress
    ) public {
        require(!isInitialized, "Operations: Already initialized");
        require(msg.sender == DACKIE_PAD_FACTORY, "Operations: Not factory");
        ...
        iDackieTier = IDackieTier(_iDackieAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        if (!_poolInformation[_pid].isSpecialSale) {
            // getUserCredit from IDackieTier contract
            uint256 idoCredit = iDackieTier.getUserTier(msg.sender);
            uint256 tierAmount;
            if (idoCredit < tier1) {
                tierAmount = tier0Amount;
            } else if (idoCredit >= tier1 && idoCredit < tier2) {
                tierAmount = tier1Amount;
            } else if (idoCredit >= tier2 && idoCredit < tier3) {
                tierAmount = tier2Amount;
            } else if (idoCredit >= tier3 && idoCredit < tier4) {
                tierAmount = tier3Amount;
            } else if (idoCredit >= tier4 && idoCredit < tier5) {
                tierAmount = tier4Amount;
            } else {
                tierAmount = tier5Amount;
            }

            require(userCreditUsed[msg.sender].add(_amount) <= tierAmount, "Not enough IDO credit left");
    ...
    function _computeReleasableAmount(VestingSchedule memory _vestingSchedule) internal view returns (uint256) {
        ...
        if (currentTime < vestingStartTime + cliffTime) {
            ...
            if (cliffTime > 0) {
                ...
                vestedSlicePeriods = timeFromStart.div(secondsPerSlice) + 1;
            } else {
                // In case there's no cliff
                vestedSlicePeriods = timeFromStart.div(secondsPerSlice);
            }
    ...
    function setTierAmounts(
        uint256 _tier0Amount,
        uint256 _tier1Amount,
        uint256 _tier2Amount,
        uint256 _tier3Amount,
        uint256 _tier4Amount,
        uint256 _tier5Amount
    ) public onlyOwner {
        tier0Amount = _tier0Amount;
        tier1Amount = _tier1Amount;
        tier2Amount = _tier2Amount;
        tier3Amount = _tier3Amount;
        tier4Amount = _tier4Amount;
        tier5Amount = _tier5Amount;
    }
}