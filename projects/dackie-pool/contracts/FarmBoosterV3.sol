...
contract FarmBoosterV3 is Ownable {
    ...
    function updateTotalLockedAmount(address _user) internal {
        uint256 totalLockedAmountInCakePool = IDackiePool(CAKE_POOL).totalLockedAmount();
        if (!initialization) {
        ...
        (, , , , , , , , uint256 userLockedAmount) = IDackiePool(CAKE_POOL).userInfo(_user);
    ...
    function _getTotalLiquidity(uint256 _pid) internal view returns (uint256) {
        ...
        uint256 v3PoolLiquidity = IDackieV3Pool(v3Pool).liquidity();
