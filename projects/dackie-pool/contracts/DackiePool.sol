...
import "./interfaces/IVDackie.sol";

contract DackiePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 cakeAtLastUserAction; // keep track of dackie deposited at the last user action.
    ...
    IERC20 public immutable token; // dackie token.
    ...
    event NewVDackieContract(address VCake);
     ...
     * @param _pid: dackie pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    /**
     * @notice Checks if the msg.sender is either the dackie owner address or the operator address.
     */
    ...
    /**
     * @notice Unlock user dackie funds.
    ...
    /**
     * @notice Deposit funds into the Dackie Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VCake != address(0)) {
            IVDackie(VCake).deposit(_user, _amount, _lockDuration);
        }
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VCake != address(0)) {
            IVDackie(VCake).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Dackie Pool
     */
     ...
     * @return Returns total pending dackie rewards
     */
    ...
    function initDepositToMCV2(IERC20 _mockToken) public onlyOwner() {
        _mockToken.approve(address(masterchefV2), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        masterchefV2.deposit(cakePoolPID, 10 * 10 ** 18);
    }
}