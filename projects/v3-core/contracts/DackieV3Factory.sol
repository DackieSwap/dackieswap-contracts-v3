...
/// @title Canonical DackieSwap V3 factory
/// @notice Deploys DackieSwap V3 pools and manages ownership and control over pool protocol fees
contract DackieV3Factory is IDackieV3Factory {
    /// @inheritdoc IDackieV3Factory
    ...
    function createPool(
        ...
        pool = IDackieV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IDackieV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IDackieV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IDackieV3Pool(pool).setLmPool(lmPool);
    }
}
