...
contract MasterChefV3KeeperV2 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    uint256 public REWARD_EMISSION_PER_DURATION = 630952 * 1e18;
    ...
    event NewRewardEmissionPerDuration(uint256 rewardEmission);
    ...
    function performUpkeep(bytes calldata) external override onlyRegister whenNotPaused {
        ...
        if (latestPeriodStartTime + upkeepBufferSecond < block.timestamp) Receiver.upkeep(REWARD_EMISSION_PER_DURATION, PERIOD_DURATION, true);
    }
    ...
    function setRewardEmissionPerDuration(uint256 _rewardEmission) external onlyOwner {
        REWARD_EMISSION_PER_DURATION = _rewardEmission;
        emit NewRewardEmissionPerDuration(_rewardEmission);
    }
