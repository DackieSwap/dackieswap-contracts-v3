...
contract FarmBooster is Ownable {
    ...
    /// @param _dackie CAKE token contract address.
    /// @param _dackiePool Cake Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the dackie pool.
    modifier onlyCakePool() {
        require(msg.sender == CAKE_POOL, "onlyCakePool: Not dackie pool");
        _;
    }
    ...
    /// @notice Dackiepool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in dackie pool.
    /// @param _lockedDuration user locked duration in dackie pool.
    /// @param _totalLockedAmount Total locked dackie amount in dackie pool.
    /// @param _maxLockDuration maximum locked duration in dackie pool.
    ...
    /// @notice dackie pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IERC20(CAKE).balanceOf(CAKE_POOL);

        uint256 totalLockedAmount = IDackiePool(CAKE_POOL).totalLockedAmount();

        uint256 pricePerFullShare = IDackiePool(CAKE_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((IDackiePool(CAKE_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = IDackiePool(CAKE_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = IDackiePool(CAKE_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration dackie pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = IDackiePool(CAKE_POOL)
            .userInfo(_user);
